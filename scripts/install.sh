...
PS3=$'\n  Selecione: '
select DE in Budgie Cinnamon Deepin GNOME LXDE LXQT MATE Plasma XFCE "Niri" "Labwc" "Hyprland"; do
  [ -n "$DE" ] && break
done
...
case "$DE" in
  ...
  Labwc)
    ...
    # Copiar labwc.sh do repo (se disponível)
    if [ -f /root/scripts/labwc.sh ]; then
      cp /root/scripts/labwc.sh "/mnt/home/$USERNAME/scripts/"
      chroot /mnt chown "$USERNAME:users" "/home/$USERNAME/scripts/labwc.sh"
      ok "labwc.sh copiado"
    fi
    ...
    ;;
  Hyprland)
    # Hyprland será instalado após reboot via hyprland.sh — sem SDDM aqui,
    # pois ele não deve iniciar antes do hyprland.sh rodar (falharia sem o Hyprland)
    arch-chroot /mnt pacman -S --noconfirm $BASE_PKGS
    arch-chroot /mnt systemctl enable NetworkManager
    ...
    warn "Após reiniciar, execute: bash ~/scripts/hyprland.sh"
    ;;
esac
